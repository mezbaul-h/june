...
from .settings.utils import process_user_settings
...
async def _real_main(**kwargs):
    ...
    llm_model, stt_model, tts_model = process_user_settings(user_config or default_config)
...
async def consumer(text_queue: asyncio.Queue[str], tts_model: Optional):
    """
    ...
    with AudioIO() as audio_io:
        while not shutdown_event.is_set():
            try:
                ...
                if tts_model:
                    file_path = tts_model.forward(text_buffer)
...
def producer(text_queue: asyncio.Queue[str], llm_model, stt_model: Optional) -> None:
    """
